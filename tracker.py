...
class Tracker(object):
    ...
    def _get_cropped_by_det(det, image):
        ...
        return image[int_det[1] - 5:int_det[3] + 5, int_det[0]:int_det[2]]
            ...
            for det, trk in zip(row, col):
                if dist_matrix[det, trk] < self.metric_threshold:
                    if self.tracks[trk].time_since_features_ext > self.tracks[trk].features_ext_interval:
                        ...
                        self.tracks[trk].extract_features(no_bg_image, h, w)
...
# Previous version - one histogram:
...
#                     det_indices = np.unique(det_indices)
#                     if len(det_indices) > 1:
#                         trk_hist = self.tracks[trk].features["hist"]
#                         crops = [Tracker._get_cropped_by_det(dets[d_i], no_bg_image) for d_i in det_indices]
#                         hists = [Track.calc_hist(cropped) for cropped in crops]
#                         diffs_hists = [Tracker._compare_hists(trk_hist, hist) for hist in hists]
#                         for i, d_i in enumerate(det_indices):
#                             if self.metric_threshold > 0:
#                                 dist_matrix[d_i, trk] *= diffs_hists[i]
#                             else:
#                                 dist_matrix[d_i, trk] *= (1-diffs_hists[i])
#                     # To visualize the score each time (when using only one hist and not up down):
...
#                             or self.tracks[trk].age == self.n_init) and (min_dist / next_min < 0.5 \
#                             and min_dist == dist_matrix[det, trk]):
#                         cropped = Tracker._get_cropped_by_det(dets[det], no_bg_image)
#                         self.tracks[trk].extract_features_from_img(cropped)
#                     self.framed_matched_track_dets.append((compiled_dets[det], self.tracks[trk]))
#         else:
#             if len(compiled_dets) > len(tracks_predicts):
#                 self.framed_unmatched_dets = [det for d, det in enumerate(compiled_dets) if d not in row]
